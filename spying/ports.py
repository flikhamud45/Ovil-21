...
def open_port(port) -> List[Tuple[str, bool, str]]:
    ...
    for path in res:
        ...
        status, message = __open_port(service_url, port, internal_client=localip, internal_port=port, protocol="TCP")
        if status == 200:
            print(f"{routerip}: succeeded!")
            ans.append((routerip, True, message))
        else:
            print(f"{routerip}: failed! - {message}")
            ans.append((routerip, False, message))
...
def __open_port(service_url,external_port,internal_client,internal_port=None,protocol='TCP',duration=10,description=None,enabled=1):
